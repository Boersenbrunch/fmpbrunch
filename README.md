...
Get prices, metrics, profiles, news, quotes, screeners, and bulk
statement data as tidy tibbles.
...
- `fmp_set_key()` is optional if `FMP_API_KEY` is already available in
  your environment.
- Most functions return a tibble and try to harmonize common fields such
  as `symbol`, dates, and numeric columns.
- News endpoints return both original API fields and standardized helper
  columns like `published_at`, `publisher_std`, `url_std`, and
  `text_std`.
